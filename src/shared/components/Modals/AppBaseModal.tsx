...
interface BaseModalProps {
  ...
  variant?: "center" | "right"; // Новий параметр
}
...
export function AppBaseModal({
  ...
  variant = "center", // За замовчуванням по центру
}: BaseModalProps) {
  ...
  // Динамічні класи залежно від варіанту
  ...
  const contentClasses = isRight
    ? `h-full w-full ${maxWidth} bg-white dark:bg-slate-900 border-l border-zinc-200 dark:border-white/10 shadow-2xl relative`
    : `modal-content w-[calc(100vw-2rem)] ${maxWidth} bg-white dark:bg-slate-900 border border-zinc-200 dark:border-white/10 rounded-2xl overflow-hidden relative shadow-2xl`;
