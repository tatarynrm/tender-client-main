      ...
      >
        ...
        <div
          className={cn(
            ...
            user.is_blocked ? "bg-red-500" : isAdmin ? "bg-rose-500" : isManager ? "bg-blue-500" : "bg-emerald-500"
          )}
        />
          ...
          <div className="md:col-span-4 min-w-0">
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                <span className="font-mono bg-zinc-50 dark:bg-zinc-800/50 px-1.5 py-0.5 rounded border border-zinc-200 dark:border-zinc-700">#{user.id}</span>
          ...
          <div className="md:col-span-4 min-w-0">
            <div className="flex flex-col">
              <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                ...
                <span className="text-[13px] font-semibold truncate">{companyName}</span>
              </div>
              <span className="text-xs text-slate-500 dark:text-slate-400 ml-5">ЄДРПОУ: {edrpou}</span>
            </div>
          </div>

          <div className="md:col-span-2">
             <div className="flex flex-col gap-1.5">
                <div className={cn("px-2 py-0.5 rounded border text-[10px] font-bold uppercase tracking-wider w-fit", roleColor)}>
                  {roleText}
                </div>
                {user.is_blocked && (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800">
                <GripVertical size={20} className="text-slate-600 dark:text-slate-400" />
              </Button>
            </DropdownMenuTrigger>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[440px] p-0 overflow-hidden border-none shadow-2xl rounded-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2">
                <label className="text-[11px] font-bold text-zinc-400 dark:text-zinc-500 uppercase ml-1 mb-1.5 block">
                  Для підтвердження введіть <span className="text-rose-500">noris</span>
                </label>
                <Input 
                  value={confirmPhrase}
                  ...
                  className={cn(
                    ...
                    confirmPhrase === "noris" && "bg-emerald-50 dark:bg-emerald-500/5 border-emerald-200 dark:border-emerald-900 focus:border-emerald-500 focus:ring-emerald-500/20 text-emerald-600 dark:text-emerald-400"
                  )}
                />
              </div>
            </div>
          ...
          <DialogFooter className="p-4 bg-zinc-50 dark:bg-zinc-800/30 border-t border-zinc-100 dark:border-zinc-800 gap-2 sm:gap-0">
            ...
            <Button 
              variant={confirmPhrase === "noris" ? "destructive" : "outline"} 
              onClick={handleDelete}
              disabled={confirmPhrase !== "noris" || isDeleting}
              className={cn(
                "rounded-xl font-bold h-11 px-6 shadow-lg shadow-rose-500/20 transition-all",
                confirmPhrase !== "noris" && "opacity-50 grayscale cursor-not-allowed shadow-none"
              )}
            >
              {isDeleting ? (
