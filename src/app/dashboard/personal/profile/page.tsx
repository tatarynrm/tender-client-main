...
import { UserRoundPen, ShieldCheck, BellRing, Settings, KeySquare, Pickaxe } from "lucide-react";

const profileTabs: TabOption[] = [
...
const getTabIcon = (id: string) => {
    switch (id) {
        case "personal":
            return <UserRoundPen className="w-8 h-8 lg:w-10 lg:h-10" />;
        case "security":
            return <ShieldCheck className="w-8 h-8 lg:w-10 lg:h-10" />;
        case "notifications":
            return <BellRing className="w-8 h-8 lg:w-10 lg:h-10" />;
        default:
        ...
        <div className="relative z-10 flex flex-col items-center text-zinc-400 dark:text-zinc-500 gap-4 transition-transform duration-500 group-hover:scale-105 group-hover:text-zinc-600 dark:group-hover:text-zinc-300">
            ...
            <div className="text-center space-y-1">
                ...
                <p className="text-xs lg:text-sm font-medium max-w-sm text-zinc-500 dark:text-zinc-400">
                    Цей розділ наразі перебуває в стадії розробки. Незабаром тут з'являться налаштування профілю.
                </p>
            </div>
